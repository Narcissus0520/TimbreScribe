...
项目处于 `v0.7.0` / Phase 6 记谱精炼与播放完善阶段。确定性的声部分配、钢琴双手拆分与人工谱表修正、三连音、打击乐非定高语义、和弦建议/人工编辑、音域诊断和节奏简化档位已实现。源音频与后台生成的轻量 PCM 乐谱预览共享同一传输时钟，并同步波形、原始/可编辑钢琴卷帘和当前乐谱音符。Phase 5 的真实 MuScriptor Small 验收仍等待用户令牌、明确条款接受和经授权本地素材；这一门禁未被跳过，也不影响其余本地功能。
...
Mock 数据始终标记为“Mock/Test”，不能代表真实音频识别结果。Basic Pitch 是乐器无关的多音高转录基线，不做乐器分离，且对单一乐器录音效果最佳。MuScriptor Small/Medium 的权重受门控且仅限非商业用途；应用不会自动下载、捆绑或替用户接受条款，真实 Small 多乐器验收仍需经明确授权的本地素材。置信度筛选只改变派生视图，不删除原始事件。速度、调号与和弦检测都是需要人工确认的建议。可选助手与发布打包仍属于后续有序里程碑。
