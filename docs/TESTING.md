...
## Phase 7 assistant safety suite

The default model-free suite tests every mutating assistant operation deterministically. It also covers strict schema rejection, out-of-preview stable IDs, explanation-only responses, local loopback/process lifecycle, credential environment scrubbing, a cloud payload with no media/archive/secret fields, non-secret settings, default-disabled UI, exact cloud-scope consent, destructive previews, explicit confirmation, undo, and invalid-output no-mutation behavior. No network request, cloud account, llama.cpp binary, or GGUF is required.
