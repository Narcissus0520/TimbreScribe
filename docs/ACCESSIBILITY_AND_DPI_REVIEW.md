...
| Dock reachability | Initial proportions prevent title-bar-only collapse; the five left workspaces use readable top-aligned short tabs with full hover descriptions and a tested 400 px minimum; MuScriptor/notation forms scroll and named `View` actions activate every dock | GUI regression test and live-window review passed |
